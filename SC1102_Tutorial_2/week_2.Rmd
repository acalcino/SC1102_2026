...
Let's plot the trend line calculated using the ``lm()`` function.
...
Let's try test out a geometric or exponential model, which you'll remember states that the rate of change is proportional to the current population size, independent of population density. First we'll visualise this on a production function ``G(N) vs N`` for the population.

```{r}
# Create an integer vector of all but the final final number of observations
...
# Geometric: G(N) = RN, a straight line through the origin with a slope calcualted from the mean of R
...
So, a negative number. This is worse than useless. It essentially means that our trendline is even worse than guessing the average, which would be a flat line at ``mean(R) * N_t``
...
Again, we could manually calculate everything we need for a geometic model of our data, or we could go with using a built in function. Let's just go with the function ``nls()`` (Nonlinear Least Squares).

```{r}
# Use the nls() function to calculate the geometric model
# N0 = N in first row, R = the estimated growth rate (37% from linear model)
geometric_fit <- nls(Paramecium ~ N0 * (1 + R)^Time..days.,
                     data = paramecium,
                     start = list(N0 = 2, R = 0.4))

summary(geometric_fit)

```

``nls()`` doesn't calculate an R2 value so we'll have to do it manually.

```{r}
# Calculate residual sum of squares
rss_geometric <- sum(residuals(geometric_fit)^2)

# Calculate total sum of squares
tss_geometric <- sum((paramecium$Paramecium - mean(paramecium$Paramecium))^2)

# Calculate R2
r2_geometric <- 1 - rss_geometric/tss_geometric
r2_geometric

```

Now let's overlay this trend line on our plot. Unfortunately this isn't quite as simple as with the linear regression
...
# Create 200 evenly spaced points on the x axis between 0 and 19
time_seq <- seq(0, 19, length.out = 200)

# Use predict() to predict the values of y at these points given our model
pred <- predict(geometric_fit, newdata = data.frame(Time..days. = time_seq))

# Join up these points to create a curve
lines(time_seq, pred, col = "blue", lwd = 2)

# Put the linear regression line on here too
abline(linear_fit, col = "red", lwd = 2)

```

Total rubbish. Let's try our third and final fit - logistic regression. Remember, 

```{r}
# Extract out all but the final (20th) row
N <- paramecium$Paramecium[-20]

# Create a numeric vector of the growth rate by dividing the difference in population size from one day to the next by the actual number on the previous day
R   <- diff(paramecium$Paramecium) / N

# Plot per capita increase (R) vs population size (N)
plot(N, R, pch = 19, xlab = "N", ylab = "R")
R_fit <- lm(R ~ N)
