...
Another great question, past me! We are in fact, going to do something and that something is produce a script that takes in a data set that you've seen before, perform a bunch of functions on it and then create a few plots. The fun part though is that at the end of this process, we're going to share the script we've written with one of our classmates to see whether or not they can run the code as it was intended, and produce the results exactly as we had done ourselves. If we can achieve this, then we can be sure that we've created reproducible code that will produce a consistent result regardless of who runs it.

First up, the data. We're going to work with the paramecium population growth data you saw in the first section of this course and you can download a copy of this from the [tutorial website](https://acalcino.github.io/SC1109_2026/). As we mentioned last week, directory structure is critically important so download this file to a logical place on your machine so we can find it easily to import it.

Now I want to introduce the concept of a ``working directory``. This is a directory that you specify, usually at the top of your script that acts like your home base. Any file in this directory will be accessible directly from R and any file outside of it can be reached using a relative or a full path. Let's set our working directory, import the data set we just downloaded and then produce a quick scatter plot to look at it. 

**SUPER ULTRA CRITICAL REMINDER HERE!!** This week we are building a script so make sure you do this in the top left panel in your script and not just in the console in the bottom left panel. If you don't create a script and instead just run all these commande in the console then you will have nothing to submit for this section of the portfolio assessment and so you will absolutely not pass!

First up, create the directory that will become your working directory and remember, your location will differ to mine!

```{r eval = FALSE}
# Create the directory you will use for your working directory
dir.create("~/Documents/teaching/2026/TR3/SC1109/tutorial_2")

```

Once this is done make sure to put the ``Paramecium_aurelia.csv`` file into it, then set your working directory and read this file into R.

```{r}
# Set working directory
setwd("~/Documents/teaching/2026/TR3/SC1109/tutorial_2")
...
Now we want to produce a simple plot to visualise the data but the issue is, we don't just want to see it in the plot window in RStudio, we also want to save a copy as an image file so that we can look at it later. To do this, we'll first plot our data using ``ggplot()`` and then we'll run the command ``ggsave()`` to save the plot to an image file.

``` {r}
# Load the ggplot2 library
library(ggplot2)

# Produce a simple scatter plot of the data using ggplot
p1 <- ggplot(paramecium, aes(x = Time..days., y = Paramecium)) +
  geom_point(size = 2) +
  labs(x = "Time (days)",
       y = "Population size (N)",
       title = "Paramecium population growth")

# Display the plot
p1

# Save the plot as an image file to our working directory.
ggsave("paramecium_growth_n_vs_time.png", plot = p1, width = 1600/300, height = 1200/300, dpi = 300)
...
While this looks like lots of fun and it actually shows the maths behind the calculations, in reality people are always going to go with using well established, built in functions in the place of writing everything out long form. 

Let's plot the trend line on an Population size (N) vs Time (days) plot calculated using the ``lm()`` function.

```{r}
# Produce the same scatter plot of the data using ggplot
p2 <- ggplot(paramecium, aes(x = Time..days., y = Paramecium)) +
  geom_point(size = 2) +
  # Overlay the trendline using the coefficients from linear_fit
  geom_abline(intercept = coef(linear_fit)[1],
...
ggsave("paramecium_growth_linear.png", plot = p2, width = 1600/300, height = 1200/300, dpi = 300)

# Check out the R2 (Multiple R2) value
summary(linear_fit)

# And let's save coefficient stats as a text file in csv format
write.csv(
...
# Put N_t and R into a data frame for ggplot
geo_df <- data.frame(N_t = N_t, R = R)

# Plot N vs R
p3 <- ggplot(geo_df, aes(x = N_t, y = R)) +
  geom_point(size = 2) +
  # Fit geometric growth model - constant R
  geom_hline(yintercept = mean(R), color = "blue") +
  labs(x = "N", y = "R", title = "Per capita growth rate vs population size")

# Display the plot
p3

# And again, save this plot as an image
ggsave("paramecium_growth_geometric_n_vs_r.png", plot = p3, width = 1600/300, height = 1200/300, dpi = 300)
...
# Put N_t and GN into a data frame for ggplot
prod_df <- data.frame(N_t = N_t, GN = GN)

# Plot N vs G(N)
p4 <- ggplot(prod_df, aes(x = N_t, y = GN)) +
  geom_point(size = 2) +
  # Geometric: G(N) = RN, a straight line through the origin with a slope calculated from the mean of R
  geom_abline(intercept = 0, slope = mean(R), color = "blue") +
  # Also plot the R2 base line at mean(GN)
  geom_hline(yintercept = mean(GN), color = "green") +
  labs(x = "N", y = "G(N)", title = "Production function")

# Display the plot
p4

# And save
ggsave("paramecium_growth_production_function.png", plot = p4, width = 1600/300, height = 1200/300, dpi = 300)
...
# Plot N vs R
p5 <- ggplot(geo_df, aes(x = N_t, y = R)) +
  geom_point(size = 2) +
  # Fit geometric growth model - constant R
  geom_hline(yintercept = mean(R), color = "blue") +
  # Fit the logistic model - R declines as population density increases
  geom_abline(intercept = coef(R_fit)[1], slope = coef(R_fit)[2], color = "darkgreen") +
  labs(x = "N", y = "R", title = "Per capita growth rate vs population size")
...
# Save
ggsave("paramecium_growth_logistic_n_vs_r.png", plot = p5, width = 1600/300, height = 1200/300, dpi = 300)
...
# Put the logistic curve into a data frame for ggplot
logistic_curve <- data.frame(
  N_seq = N_seq,
  G_logistic = R0 * (1 - N_seq/K) * N_seq
)

# Plot N vs G(N)
p6 <- ggplot(prod_df, aes(x = N_t, y = GN)) +
  geom_point(size = 2) +
  # Geometric: G(N) = RN, a straight line through the origin with a slope calculated from the mean of R
  geom_abline(intercept = 0, slope = mean(R), color = "blue") +
  # Now plot the line using the logistic growth equation
  geom_line(data = logistic_curve, aes(x = N_seq, y = G_logistic), color = "darkgreen") +
  # Include a horizontal at G(N) = 0
  geom_hline(yintercept = 0, color = "grey40") +
  # Include vertical lines at K and K/2
  geom_vline(xintercept = K, color = "grey40", linetype = "dashed") +
  geom_vline(xintercept = K/2, color = "grey40", linetype = "dashed") +
  labs(x = "N", y = "G(N)", title = "Production function")
...
# Save
ggsave("paramecium_growth_all_models_gn_vs_n.png", plot = p6, width = 1600/300, height = 1200/300, dpi = 300)
...
Just to satisfy yourself that you understand what's going on here, try and calculate the second value in this vector using the same formula ``i + 1 = i + mean(R) * i``. In our case, when ``i`` = 1, geometric_pred equals 2.
...
Then finally test whether this matches the value of geometric_pre[2]:

```{r}
geometric_pred[2] == 2 + mean(R) * 2

```

Alright, now that we have these predicted population growth rates, lets plot them to see how they compare to our observed population size numbers.

```{r}
# Put the geometric and logistic predictions into a data frame for ggplot
pred_df <- data.frame(
  Time = 0:19,
  Geometric = geometric_pred,
  Logistic = logistic_pred
)

# Produce the same scatter plot of the data
p7 <- ggplot(paramecium, aes(x = Time..days., y = Paramecium)) +
  geom_point(size = 2) +
  # First, fit the linear model
  geom_abline(aes(intercept = coef(linear_fit)[1], slope = coef(linear_fit)[2], color = "Linear"),
              linewidth = 1) +
  # Next, the geometric model predicted curve
  geom_line(data = pred_df, aes(x = Time, y = Geometric, color = "Geometric"), linewidth = 1) +
  # And finally the logistic model predicted curve
  geom_line(data = pred_df, aes(x = Time, y = Logistic, color = "Logistic"), linewidth = 1) +
  # Add a legend
  scale_color_manual(name = NULL, values = c("Linear" = "red", "Geometric" = "blue", "Logistic" = "darkgreen")) +
  # Add labels
  labs(x = "Time (days)", y = "Population size (N)", title = "Paramecium population growth") +
  # Limit y axis to 1000 as the geometric line gets out of control by day 19
...
# Save
ggsave("paramecium_growth_all_models_n_vs_time.png", plot = p7, width = 1600/300, height = 1200/300, dpi = 300)

```

And finally, lets see which one fits the data best. We'll put the results in a dataframe and then save it as a ``.csv``.
...
# Calculate R² values
r2_linear <- 1 - sum((paramecium$Paramecium - fitted(linear_fit))^2) / tss_N
r2_geometric <- 1 - sum((paramecium$Paramecium - geometric_pred)^2) / tss_N
r2_logistic <- 1 - sum((paramecium$Paramecium - logistic_pred)^2) / tss_N

# Create table
r2_results <- data.frame(
  Model = c("Linear", "Geometric", "Logistic"),
  R_squared = c(r2_linear, r2_geometric, r2_logistic)
)
...
# Save as CSV
write.csv(r2_results,
          "model_r2_results.csv",
          row.names = FALSE)
